...
import numpy as np
from learn_facinh import learning
...
num_cycle=40 # number of cycles/years
countyears=1 # which years to include in the tables
num_birds=100 # number of birds/individuals
learningoccasions = 30 # number of learning occasions per year
#mr=[0.2, 0.4, 0.8] # mutation or variability rate
mr=[0.6]
#alpha=[-0.01,-0.005,-0.001, 0, 0.001, 0.005, 0.01] # alpha, the weight of learning
alpha=[0.005]
syllget=0 # How new individuals get their syllable repertoire at the start of the simulations:
#           0 meaning randomly, and 1 meaning from an exponential distribution
for m_r in mr:
    for a in alpha:
        ...
        ind1=sample(birds, 1) # a randomly chosen individual
        if a>0:
            learnmode='fac0'+str(a)[2:]
        elif a<0:
            learnmode='inh0'+str(a)[3:]
        else:
            learnmode='0'
        ...
        for c in range(num_cycle):
            ...
            for lo in range(learningoccasions):
                for i in range(len(birds)):
                    oneneigh8(i, birds, bird_grid)
                    if len(birds[i].neighbours)==0:
                        oneneigh24(i, birds, bird_grid)
                    if len(birds[i].neighbours)==0:
            ...
            # learning:
                ...
                if ind1[0] in birds and ind1[0].age==0:
                    #print(ind1[0] in birds, ind1[0].age)
            ...
            # we need the rel_freq of each syllable in the whole population
                ...
                # The whole populations' song: pop_rep
        ...
        #renewing the site:
            ...
            birds=survivors+new
            #print(len(ind1_rep))
        # Creating the data.frames for the analysis and data visualization:
            """
      ...
      """
        ...
        diff=[]
        values=[]
        for col in range(num_cycle-10,num_cycle):
            for r in range(num_cycle-10,num_cycle):
                if col>r:
                    diff.append(col-r)
                    values.append(dmp.iloc[r,col])
        lydmp[1]=diff
        lydmp[2]=values
